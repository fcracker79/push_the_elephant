...
")           
    ...
    if let Some(yaml_file) = matches.value_of("yaml_file") {
        return push_the_elephant::Worker::multi_run(yaml_file).expect("Could not multi run!");
    }
